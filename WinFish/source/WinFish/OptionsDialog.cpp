...
// Baked in at build time from the PORT_VERSION file (see CMakeLists.txt).
#ifndef WINFISH_PORT_VERSION
#define WINFISH_PORT_VERSION "dev"
#endif
...
void Sexy::OptionsDialog::Draw(Graphics* g)
{
	...
	// Real port version, centered horizontally and vertically in the gap between
	// the OK button and the dialog's bottom border. The title screen keeps its
	// "MacOS lol" easter egg (empty mProductVersion); this is a separate, honest
	// version string the in-app updater tracks releases by.
	auto* aVerFont = g->GetFont();
	SexyString aVer = SexyString("v") + WINFISH_PORT_VERSION;
	int aOkBottom  = (mYesButton->mY - mY) + mYesButton->mHeight;
	// Center in the visible gap between the OK button and the bottom frame:
	// the outer edge (mHeight) sits on the wood border, so pull up a quarter of
	// the content inset from that midpoint.
	int aGapCenter = (aOkBottom + mHeight) / 2 - mContentInsets.mBottom / 4;
	g->SetColor(Color(150, 150, 150));
	g->DrawString(aVer,
		(mWidth - aVerFont->StringWidth(aVer)) / 2,
		aGapCenter + (aVerFont->GetAscent() - aVerFont->GetDescent()) / 2);
}

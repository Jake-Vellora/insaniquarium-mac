...
void TitleScreen::Draw(Graphics* g)
{
	...
	g->SetFont(FONT_JUNGLEFEVER10OUTLINE);
	g->SetColor(Color::White);
	// The Steam build ships an empty product version, so the title just read
	// "Version". Native macOS port easter egg. :)
	SexyString aVersionStr = mApp->mProductVersion.empty()
		? SexyString("MacOS lol")
		: SexyString("Version " + mApp->mProductVersion);
...
{
	Widget::Update();
	if (getenv("INSANIQ_TITLEDEBUG") && (mUpdateCnt % 60) == 0)
		fprintf(stderr, "TITLEDEBUG cnt=%d m0x94=%d m0x98=%d stinkyX=%d m0xa1=%d loadDone=%d progress=%f scr=%d\n",
			mUpdateCnt, m0x94, m0x98, mStinkyX, (int)m0xa1,
			(int)gSexyApp->mLoadingThreadCompleted, mApp->GetLoadingThreadProgress(), (int)mApp->IsScreenSaver());

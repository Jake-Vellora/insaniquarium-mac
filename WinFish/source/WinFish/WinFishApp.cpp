...
WinFishApp::WinFishApp()
{
	...
	// Default to fullscreen on macOS (a persisted ScreenMode in the registry
	// still overrides this once the player picks a mode in Options).
	mIsWindowed = false;
...
void Sexy::WinFishApp::ButtonDepress(int theId)
{
	...
	if (anIdVar1 == 10006)
	{
		// "Yes" on the New Version dialog -> download + install via the shell
		// updater, quit, relaunch.
		KillDialog(10006);
		KillDialog(DIALOG_UPDATE_CHECK);
		BeginInAppUpdate();
		return;
...
void Sexy::WinFishApp::ApplyScreenSaverSettings()
{
	...
	// On macOS the game does not install itself as the system screensaver
	// (that is the separate Insaniquarium.saver bundle, chosen in System
	// Settings). Skip the Windows .scr copy/registry-install machinery; just
	// record the enabled flag and persist the display preferences below, which
	// the native .saver reads from the shared registry file.
	mScreenSaverEnabled = aDia->mSSEnabledCB->mChecked;
...
void Sexy::WinFishApp::DoUpdateDialog()
{
	...
	// Record that we asked now, so answering either Yes or No defers the weekly
	// prompt (ShouldCheckForUpdate rate-limits off this timestamp).
	UpdateCheckQueried();
}
...
void Sexy::WinFishApp::DoUpdateCheckDialog()
{
	...
	// Kick off the async check against GitHub Releases; the spinner dialog polls
	// UpdateCheck::Poll() each frame and shows the up-to-date / new-version result.
	UpdateCheck::Start();
	UpdateCheckQueried(); // also covers the Options-menu "check now" path
}

void Sexy::WinFishApp::BeginInAppUpdate()
{
	// A running app can't replace its own bundle, so hand off to the detached
	// shell updater ($PORTHOME/in-app-update.sh), then quit; it swaps the app
	// while we're gone and relaunches us. Gated to real installed ports.
	if (!UpdateCheck::IsUpdatableInstall() || !UpdateCheck::SpawnUpdater(getpid()))
	{
		DoDialog(DIALOG_INFO, true, "Update",
			"Couldn't start the updater automatically.\n\n"
			"To update, quit and run this in Terminal:\n"
			"curl -fsSL https://github.com/Jake-Vellora/insaniquarium-mac/"
			"releases/latest/download/update.sh | bash",
			"OK", Dialog::BUTTONS_FOOTER);
		return;
	}

	DoDialog(DIALOG_INFO, true, "Updating",
		"Insaniquarium will now close to install the update,\nthen reopen automatically.",
		"", Dialog::BUTTONS_NONE);
	DrawDirtyStuff(); // paint the notice before we tear down
	Shutdown();
}

	...
	if (SDL_PollEvent(&event))
	{
		switch(event.type)
		{
			...
			case SDL_MOUSEBUTTONDOWN:
			{
				...
				mWidgetManager->RemapMouse(x, y);
				if (getenv("INSANIQ_INPUTDEBUG"))
					fprintf(stderr, "INPUTDEBUG down raw=(%d,%d) remap=(%d,%d) src=(%d,%d %dx%d) dst=(%d,%d %dx%d)\n",
						event.button.x, event.button.y, x, y,
						mWidgetManager->mMouseSourceRect.mX, mWidgetManager->mMouseSourceRect.mY,
						mWidgetManager->mMouseSourceRect.mWidth, mWidgetManager->mMouseSourceRect.mHeight,
						mWidgetManager->mMouseDestRect.mX, mWidgetManager->mMouseDestRect.mY,
						mWidgetManager->mMouseDestRect.mWidth, mWidgetManager->mMouseDestRect.mHeight);

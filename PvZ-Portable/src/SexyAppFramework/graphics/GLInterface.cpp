...
#else
	int width, height;
	if (gGLHostDrawableWidth > 0 && gGLHostDrawableHeight > 0)
	{
		width = gGLHostDrawableWidth;
		height = gGLHostDrawableHeight;
	}
	else
...
int GLInterface::Init(bool IsWindowed)
{
	...
	if (!inited || gGLForceContextReinit)
	{
		inited = true;
		gGLForceContextReinit = false;
		PlatformGLInit();
...
#if !defined(NINTENDO_SWITCH) && !defined(__EMSCRIPTEN__)
// INSANIQ_AUTOSHOT="<path.png>:<frame>": dump the completed back buffer to a PNG
// at the given frame and exit 0. Used for automated render verification.
#include "imagelib/ImageLib.h"
static void AutoShotHook(SDL_Window* theWindow)
{
	static int aShotFrame = -2;
	static int aFrameNum = 0;
	static std::string aShotPath;
	if (aShotFrame == -2)
	{
		aShotFrame = -1;
		const char* aSpec = std::getenv("INSANIQ_AUTOSHOT");
		if (aSpec != nullptr)
		{
			std::string aStr = aSpec;
			std::string::size_type aColon = aStr.rfind(':');
			aShotPath = aStr.substr(0, aColon);
			aShotFrame = (aColon == std::string::npos) ? 60 : atoi(aStr.c_str() + aColon + 1);
		}
	}
	if (aShotFrame < 0 || aFrameNum++ != aShotFrame)
		return;

	int w = 0, h = 0;
	if (gGLHostDrawableWidth > 0 && gGLHostDrawableHeight > 0)
	{
		w = gGLHostDrawableWidth;
		h = gGLHostDrawableHeight;
	}
	else
		SDL_GL_GetDrawableSize(theWindow, &w, &h);
	std::vector<uint32_t> aBuf(w * h);
	glReadPixels(0, 0, w, h, GL_RGBA, GL_UNSIGNED_BYTE, aBuf.data());

	ImageLib::Image anImage;
	anImage.mWidth = w;
	anImage.mHeight = h;
	anImage.mBits = new uint32_t[w * h];
	for (int y = 0; y < h; y++)
	{
		const uint32_t* src = aBuf.data() + (h - 1 - y) * w;
		uint32_t* dst = anImage.mBits + y * w;
		for (int x = 0; x < w; x++)
			dst[x] = RgbaToArgb(src[x]) | 0xFF000000u;
	}
	bool aWrote = ImageLib::WritePNGImage(aShotPath, &anImage);
	fprintf(stderr, "INSANIQ_AUTOSHOT: %s frame %d -> %s\n",
		aWrote ? "wrote" : "FAILED TO WRITE", aShotFrame, aShotPath.c_str());
	exit(aWrote ? 0 : 3);
}
#endif

// Saver host hooks: external GL host (ScreenSaverView) supplies swap +
// drawable size because there is no SDL window in that build.
namespace Sexy
{
	void (*gGLSwapHook)() = nullptr;
	int gGLHostDrawableWidth = 0;
	int gGLHostDrawableHeight = 0;
	long gGLFlushCount = 0;
	// Shader programs are NOT shared across a GL share group; when a saver
	// host moves the game to a new context, Init must recompile them.
	bool gGLForceContextReinit = false;
}
...
#else
	AutoShotHook((SDL_Window*)mApp->mWindow);
	if (gGLSwapHook != nullptr)
		gGLSwapHook();
	else
		SDL_GL_SwapWindow((SDL_Window*)mApp->mWindow);
#endif
#ifndef __EMSCRIPTEN__
	// Clear back buffer after swap (content undefined). Skip under an
	// external GL host: its layer presents AFTER this returns, so clearing
	// here would erase the frame just drawn; the host clears before drawing.
	if (gGLSwapHook == nullptr)
		glClear(GL_COLOR_BUFFER_BIT);

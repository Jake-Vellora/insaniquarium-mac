...
family-shared) downloads the game files during setup — that's what the
"Install click" step is for.
...
2. Drag this folder's `setup.sh` into the Terminal window — or type
   `bash ` (with the trailing space) and then drag it — and press **Return**.
